...
window.com_company_untitled16_web_ui_components_jscomponent_GridDashboard = function () {
  ...
  function ensureContextMenu() {
    ...
    div.addEventListener('click', function (ev) {
      ...
      if (act === 'add') openPicker();
      else if (act === 'reset') resetLayoutToDefaults();
      else if (act === 'lock') setLocked(!isLocked);
    });
  ...
  // ===== PELL deps (local, no CDN) =====
  function loadCssOnce(href) {
    if (document.querySelector('link[data-dash-href="' + href + '"]')) return;
    var link = document.createElement('link');
    link.rel = 'stylesheet';
    link.href = href;
    link.setAttribute('data-dash-href', href);
    document.head.appendChild(link);
  }

  function loadScriptOnce(src, cb) {
    if (document.querySelector('script[data-dash-src="' + src + '"]')) { cb && cb(); return; }
    var s = document.createElement('script');
    s.src = src;
    s.async = true;
    s.onload = function () { cb && cb(); };
    s.onerror = function () { console.error('[dash] failed to load', src); };
    s.setAttribute('data-dash-src', src);
    document.head.appendChild(s);
  }

  function ensurePellReady(cb) {
    loadCssOnce('/VAADIN/pell/pell.css');
    if (window.pell && window.pell.init) { cb && cb(); return; }
    loadScriptOnce('/VAADIN/pell/pell.min.js', cb);
  }

  function pellKey(widgetId) {
    return 'dash.pell.v1.' + widgetId;
  }
  ...
  function defaultRectById(id) {
    switch (id) {
      ...
      case 'widget-pell':     return { x: 0, y: 12, w: 6, h: 5 };
  ...
  function setRootHtmlOnce() {
    ...
    element.classList.add('dash-root', 'tezis-skin');
  ...
  // ===== tile building =====
  function makeTile(id, title, bodyHtml, canRemove, headerActionsHtml) {
    var actions = headerActionsHtml ? (headerActionsHtml + '') : '';
    ...
    return '' +
      ...
      '      <span class="dash-hdr-actions">' + actions + '</span>' +
      removeBtn +
      '    </div>' +
  ...
  function addWidget(id, rect, fromRestore) {
    ...
    if (id === 'widget-manager') {
      ...
      title = 'Новости';
      headerActionsHtml =
        '<button type="button" class="dash-btn dash-icon-btn dash-news-refresh" title="Обновить" aria-label="Обновить">' +
        '  <svg class="dash-ico" viewBox="0 0 24 24" aria-hidden="true">' +
        '    <path d="M17.65 6.35A7.95 7.95 0 0 0 12 4V1L7 6l5 5V7a6 6 0 1 1-6 6H4a8 8 0 1 0 13.65-6.65z"></path>' +
        '  </svg>' +
        '</button>';
      bodyHtml = '<div class="dash-news-list"></div>';

    } else if (id === 'widget-recent') {
      title = 'Последние документы';
      headerActionsHtml =
        '<button type="button" class="dash-btn dash-icon-btn dash-recent-refresh" title="Обновить" aria-label="Обновить">' +
        '  <svg class="dash-ico" viewBox="0 0 24 24" aria-hidden="true">' +
        '    <path d="M17.65 6.35A7.95 7.95 0 0 0 12 4V1L7 6l5 5V7a6 6 0 1 1-6 6H4a8 8 0 1 0 13.65-6.65z"></path>' +
        '  </svg>' +
        '</button>';
      bodyHtml = '<div class="dash-recent-list"></div>';

    } else if (id === 'widget-pell') {
      title = 'Заметка';
      bodyHtml =
        '<div class="dash-pell-head">' +
        '  <button type="button" class="dash-btn dash-pell-clear">Очистить</button>' +
        '</div>' +
        '<div class="dash-pell-host"></div>';

    } else {
      bodyHtml = '<div>Unknown widget</div>';
    }

    $grid[0].insertAdjacentHTML('beforeend', makeTile(id, title, bodyHtml, canRemove, headerActionsHtml));
  ...
  function resetLayoutToDefaults() {
    ...
    ['widget-clock', 'widget-calendar', 'widget-notes', 'widget-news', 'widget-recent', 'widget-pell'].forEach(function (x) {
    ...
    // По умолчанию НЕ добавляем заметку (можно включить, если хочешь)
    // addWidget('widget-pell', defaultRectById('widget-pell'), false);
  ...
  function injectPickerStylesOnce() {
    ...
    var css = [
      ...
      '.dash-handle{flex:0 0 auto;display:flex;align-items:center;gap:10px;}',
      '.dash-title{font-weight:800;}',
      '.dash-hdr-actions{margin-left:auto;display:inline-flex;gap:8px;align-items:center;}',

      // muted
      '.dash-muted{padding:10px;opacity:.70;font-size:13px;}',
      ...
      // NOTES list
      '.dash-note-row{padding:10px;border-bottom:1px solid rgba(0,0,0,.06);display:flex;gap:10px;align-items:flex-start;cursor:pointer;}',
      '.dash-note-row:hover{background:rgba(0,0,0,.03);}',
      '.dash-note-text{flex:1;min-width:0;}',
      '.dash-note-del{border:1px solid rgba(0,0,0,.14);background:#fff;border-radius:10px;padding:6px 10px;font-weight:700;cursor:pointer;}',
      '.dash-note-del:hover{background:rgba(0,0,0,.06);}',
      ...
      // icon button
      '.dash-icon-btn{padding:6px 8px;min-width:34px;display:inline-flex;align-items:center;justify-content:center;}',
      '.dash-ico{width:18px;height:18px;display:block;fill:currentColor;}',
      ...
      // PELL
      '.dash-pell-head{padding:8px;display:flex;justify-content:flex-end;gap:8px;}',
      '.dash-pell-host{height:100%;display:flex;flex-direction:column;}',
      '.dash-pell-host .pell-actionbar{flex:0 0 auto;position:sticky;top:0;z-index:2;background:#fff;}',
      '.dash-pell-host .pell-content{flex:1;min-height:0;overflow:auto;padding:8px;border:1px solid rgba(0,0,0,.10);border-radius:10px;}'
    ].join('\n');
  ...
  function ensurePicker() {
    ...
    var html =
      ...
      '        <div class="dash-widget-action">Добавить</div>' +
      '      </div>' +
      '      <div class="dash-widget-card" data-widget="widget-pell">' +
      '        <div class="dash-widget-icon">✍️</div>' +
      '        <div><div class="dash-widget-title">Заметка</div><div class="dash-widget-desc">Простой rich-text (Pell)</div></div>' +
      '        <div class="dash-widget-action">Добавить</div>' +
      '      </div>' +
      '    </div>' +
      '  </div>' +
  ...
  function bindTileLogic(id) {
    ...
    if (id === 'widget-manager') {
      ...
      btnGE && btnGE.addEventListener('click', function (e) {
        e.preventDefault(); e.stopPropagation();
        if (connector.sendTestGlobalEvent) connector.sendTestGlobalEvent('Пинг: ' + new Date().toLocaleString());
        else alert('sendTestGlobalEvent не реализован в Java');
      });
    ...
    if (id === 'widget-pell') initPellWidget();
  }
  ...
  function fitClock() {
    ...
    var reserve = 0;
    if (dateEl) reserve = Math.min(40, Math.max(16, dateEl.offsetHeight || 18)) + 4;
  ...
  function initNewsWidget() {
    ...
    function load() {
      if (list) list.innerHTML = '<div class="dash-muted">Загрузка...</div>';
      if (connector.requestNews) connector.requestNews();
      else if (list) list.innerHTML = '<div class="dash-muted">requestNews не реализован в Java</div>';
    }
  ...
  function initRecentWidget() {
    ...
    function load() {
      ...
      else if (list) list.innerHTML = '<div class="dash-muted">requestRecentDocs не реализован в Java</div>';
    }
  ...
  // ===== PELL widget (обычный, без лишнего) =====
  function initPellWidget() {
    var tile = element.querySelector('#widget-pell');
    if (!tile) return;

    var host = tile.querySelector('.dash-pell-host');
    var btnClear = tile.querySelector('.dash-pell-clear');
    if (!host) return;

    host.addEventListener('mousedown', function (e) { e.stopPropagation(); }, true);
    host.addEventListener('touchstart', function (e) { e.stopPropagation(); }, true);

    // "Очистить" должно работать всегда (даже если pell ещё не загрузился)
    if (btnClear && !btnClear.getAttribute('data-bind')) {
      btnClear.setAttribute('data-bind', '1');
      btnClear.addEventListener('click', function (e) {
        e.preventDefault(); e.stopPropagation();
        var ed = host._pellEditor;
        if (ed && ed.content) ed.content.innerHTML = '';
        try { localStorage.removeItem(pellKey('widget-pell')); } catch (ex) {}
      });
    }

    ensurePellReady(function () {
      try {
        if (host.getAttribute('data-pell-inited') === '1') return;
        host.setAttribute('data-pell-inited', '1');

        var saved = '';
        try { saved = localStorage.getItem(pellKey('widget-pell')) || ''; } catch (e) { saved = ''; }

        var editor = window.pell.init({
          element: host,
          onChange: function (html) {
            try { localStorage.setItem(pellKey('widget-pell'), html); } catch (e) {}
          },
        ...
        host._pellEditor = editor;
        if (editor && editor.content) editor.content.innerHTML = saved;

      } catch (err) {
        console.error('[dash] pell init failed', err);
      }
    });
  }
  ...
  connector.initDashboard = function () {
    ...
    (function tick() {
      ...
      if (!grid) {
        ...
        if (typeof grid.on === 'function') grid.on('change', function () { saveLayout(); });
      ...
      if (!ok || !hasAny) {
        ...
        // По умолчанию НЕ добавляем заметку
        // addWidget('widget-pell', defaultRectById('widget-pell'), true);
        saveLayout();
      }
